...
/* declarations of helper functions (only to change function order) */
...
commandStruct *getCommandFromUser(gameParams *game) {
    ...
    do {
        ...
        if (fgets(input, COMMAND_LEN, stdin) == NULL) { exitCleanly(); } /* TODO reached EOF - exit cleanly - to be implemented */
